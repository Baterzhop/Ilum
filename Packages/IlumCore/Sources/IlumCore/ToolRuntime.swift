...
public struct ToolDescriptor: Equatable, Codable, Sendable {
    ...
    public init(
        ...
        self.name = name; self.version = version; self.summary = summary
        self.risk = risk; self.capability = capability; self.inputSchema = inputSchema
    }

    public var registryKey: String { "\(name)@\(version)" }
    public var wireName: String { Self.makeWireName(name: name, version: version) }
    ...
    public static func makeWireName(name: String, version: String) -> String {
        func sanitize(_ value: String) -> String {
            ...
            for scalar in value.unicodeScalars {
                ...
                let allowed = (48...57).contains(ascii) || (65...90).contains(ascii) ||
                    (97...122).contains(ascii) || ascii == 45 || ascii == 95
                if allowed {
                    output.unicodeScalars.append(scalar); previousWasSeparator = false
                } else if !previousWasSeparator {
                    output.append("_"); previousWasSeparator = true
                }
            }
        ...
        return String(base.prefix(max(1, 64 - suffix.count))) + suffix
    }
}
...
public extension Tool {
    func permissionRequest(for input: Input) throws -> PermissionRequest {
        PermissionRequest(capability: Self.descriptor.capability, resource: try resource(for: input), reason: Self.descriptor.summary)
    }
    func warnings(for input: Input, output: Output) -> [ToolWarning] { [] }
    func metadata(for input: Input, output: Output) -> [String: JSONValue] { [:] }
}
...
public struct ToolCall: Identifiable, Equatable, Codable, Sendable {
    ...
    public static func encoding<Input: Encodable & Sendable>(
        name: String, version: String, input: Input, providerCallID: String? = nil
    ) throws -> ToolCall {
        ToolCall(providerCallID: providerCallID, name: name, version: version, arguments: try JSONEncoder().encode(input))
    }
}
...
public struct ToolExecutionSuccess: Sendable, Equatable {
    ...
    public init(callID: UUID, descriptor: ToolDescriptor, data: JSONValue, warnings: [ToolWarning] = [], metadata: [String: JSONValue] = [:]) {
        self.callID = callID; self.descriptor = descriptor; self.data = data
        self.warnings = warnings; self.metadata = metadata
    }
}
...
public enum ToolRuntimeError: Error, CustomStringConvertible, Sendable {
    ...
    public var description: String {
        switch self {
        case .duplicateTool(let key): return "Tool registry contains duplicate tool \(key)."
        case .duplicateWireName(let name): return "Tool registry contains duplicate model wire name \(name)."
        case .unknownTool(let name, let version): return "Unknown tool \(name)@\(version)."
        case .invalidArguments(let tool, let details): return "Invalid arguments for \(tool): \(details)"
        case .invalidOutput(let tool, let details): return "Invalid output from \(tool): \(details)"
        }
    }
}
...
public struct AnyTool: Sendable {
    ...
    public init<T: Tool>(_ tool: T) {
        ...
        permissionResolver = { data in
            ...
            } catch let error as ToolRuntimeError { throw error }
            catch { throw ToolRuntimeError.invalidArguments(tool: T.descriptor.registryKey, details: String(describing: error)) }
        }
        executor = { data in
            let input: T.Input
            do { input = try JSONDecoder().decode(T.Input.self, from: data) }
            catch { throw ToolRuntimeError.invalidArguments(tool: T.descriptor.registryKey, details: String(describing: error)) }
            let output = try await tool.execute(input)
            do {
                let encoded = try JSONEncoder().encode(output)
                let structured = try JSONDecoder().decode(JSONValue.self, from: encoded)
                return ErasedToolResult(data: structured, warnings: tool.warnings(for: input, output: output), metadata: tool.metadata(for: input, output: output))
            } catch { throw ToolRuntimeError.invalidOutput(tool: T.descriptor.registryKey, details: String(describing: error)) }
        }
    }

    func permissionRequest(arguments: Data) throws -> PermissionRequest { try permissionResolver(arguments) }
    fileprivate func execute(arguments: Data) async throws -> ErasedToolResult { try await executor(arguments) }
}
...
public struct ToolRegistry: Sendable {
    ...
    public init(tools: [AnyTool]) throws {
        ...
        for tool in tools {
            ...
            guard indexed[key] == nil else { throw ToolRuntimeError.duplicateTool(key) }
            guard wireNames.insert(tool.descriptor.wireName).inserted else { throw ToolRuntimeError.duplicateWireName(tool.descriptor.wireName) }
            indexed[key] = tool
        }
        self.tools = indexed
    }
    public func resolve(name: String, version: String) -> AnyTool? { tools["\(name)@\(version)"] }
    public var descriptors: [ToolDescriptor] { tools.values.map(\.descriptor).sorted { $0.registryKey < $1.registryKey } }
}
