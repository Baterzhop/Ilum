...
public actor KnowledgeIngestionEngine {
    ...
    public init(extractor: any DocumentTextExtractor, store: any KnowledgeStore, chunker: KnowledgeChunker = KnowledgeChunker()) {
        self.extractor = extractor; self.store = store; self.chunker = chunker
    }

    @discardableResult
    public func ingest(resourceID: UserFileResourceID) async throws -> KnowledgeIngestionResult {
        let extracted = try await extractor.extract(resourceID: resourceID)
        guard extracted.sourceResourceID == resourceID else { throw KnowledgeIngestionError.sourceIdentityMismatch }
        guard !extracted.pages.isEmpty else { throw KnowledgeIngestionError.emptyDocument }
        ...
        let document = KnowledgeDocument(
            id: documentID, sourceResourceID: resourceID, displayName: extracted.displayName,
            mediaType: extracted.mediaType, pageCount: pageCount, metadata: extracted.metadata,
            createdAt: existing?.createdAt ?? now, updatedAt: now
        )
        let chunks = try chunker.chunk(extracted, documentID: documentID)
        guard !chunks.isEmpty else { throw KnowledgeIngestionError.noChunks }
