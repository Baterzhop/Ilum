...
public enum EmbeddingError: Error, CustomStringConvertible, Sendable {
...
public struct OllamaEmbeddingProvider: EmbeddingProvider, Sendable {
    ...
    public init(endpoint: URL? = nil, model: String? = nil, timeout: TimeInterval = 60) {
        let e = ProcessInfo.processInfo.environment
        self.endpoint = endpoint ?? URL(string: e["ILUM_OLLAMA_EMBED_URL"] ?? "http://127.0.0.1:11434/api/embed")!
        self.modelID = model ?? e["ILUM_EMBED_MODEL"] ?? "nomic-embed-text"
    ...
    public func embed(_ texts: [String]) async throws -> [[Float]] {
        ...
        struct ResponseBody: Decodable { let embeddings: [[Double]]? }
        ...
        guard let raw = decoded.embeddings, raw.count == texts.count, !raw.isEmpty else { throw EmbeddingError.emptyEmbedding }
        let vectors = raw.map { $0.map(Float.init) }
        try Self.validate(vectors)
        return vectors
    }

    private static func validate(_ vectors: [[Float]]) throws {
        guard let dimension = vectors.first?.count, dimension > 0 else { throw EmbeddingError.emptyEmbedding }
        for vector in vectors {
            guard vector.count == dimension else { throw EmbeddingError.inconsistentDimensions }
            guard vector.allSatisfy({ $0.isFinite }) else { throw EmbeddingError.invalidVector }
        }
    }
}
